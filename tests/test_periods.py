...
def test_eurostat_historical_quarter_label() -> None:
    period = period_from_label("1980-Q1", "quarterly")
    assert period.label == "1980-Q1"
    assert period.start.isoformat() == "1980-01-01"
    assert period.end.isoformat() == "1980-03-31"


def test_eurostat_semiannual_label() -> None:
    period = period_from_label("2025-S2", "semiannual")
    assert period.label == "2025-S2"
    assert period.start.isoformat() == "2025-07-01"
    assert period.end.isoformat() == "2025-12-31"
